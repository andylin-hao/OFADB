...
​		简单来说，我们采用数据表的形式对元数据进行了持久化，可通过文件持久化保存元数据，并能根据文件在内存中重建元数据在内存中的表示。我们支持的功能包括
...
​		我们使用与普通数据表相同的方式管理元数据。所有的元数据都存在数据表中，所有的源数据表构成了一个系统数据库（System）。具体来说，我们将元数据分为三张表——数据库元数据表、表元数据表、索引源数据表，分别存储数据库、表、索引相关的元数据信息。三张表存储的元数据实际内容和类型如下。
...
- 在索引元数据表中删除待删除表的所有索引对应的表项
- 在表元数据表中删除待删除表对应的表项
- 非元数据操作

#### 删除数据库

- 执行隶属于该数据库的所有数据表的删除操作
- 删除待删除数据库在数据库元数据表的表项
- 非元数据操作
